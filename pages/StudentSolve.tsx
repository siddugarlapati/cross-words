...
const StudentSolve: React.FC = () => {
    ...
    const loadData = async (code: string) => {
        ...
        try {
            ...
            // Calculate grid dimension bounds
            let maxR = 0, maxC = 0;
            ...
            setError('Failed to connect to assessment database.');
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleStart = async () => {
        ...
        try {
            const hasSubmitted = await db.hasStudentSubmitted(assessment!.id, rollNumber.trim().toUpperCase());
            if (hasSubmitted) {
                alert('You have already submitted a response for this assessment.');
                return;
            }
        } catch (e) {
            console.warn('Submission check skipped:', e);
        }

        // Generate student-unique layout & question shuffle from roll number seed
        let seed = stringToSeed(rollNumber.trim().toUpperCase() + (assessment?.id || ''));
        const seededRandom = () => {
            const x = Math.sin(seed++) * 10000;
            return x - Math.floor(x);
        };

        const studentQuestionsPool = [...questions].sort(() => seededRandom() - 0.5);
        const studentWords = studentQuestionsPool.map(q => ({ word: q.word, clue: q.clue }));
        const studentLayout = generateLayout(studentWords, studentWords.length);

        let finalQuestions: Question[] = [];
        if (studentLayout.length > 0) {
            ...
            finalQuestions = studentQuestionsPool;
        }

        setQuestions(finalQuestions);

        // Recalculate grid dimensions for student layout
        let maxR = 0, maxC = 0;
    ...
    const handleCellChange = (r: number, c: number, val: string) => {
        const char = val.toUpperCase().replace(/[^A-Z]/g, '');
        const updated = [...gridData.map(row => [...row])];
        updated[r][c] = char || '_';
        setGridData(updated);

        // Auto focus to next cell if letter typed
        if (char && focusedCell) {
            moveFocus(1);
        }
    };
    ...
    const handleSubmit = async (isAutoSubmit = false) => {
        ...
        try {
            ...
            if (studentEmail.trim()) {
                ...
                }).catch(e => console.warn('Student email trigger failed:', e));
            }
            ...
            if (assessment?.faculty_email) {
                emailService.sendFacultyReportEmail({
                    ...
                    totalQuestions: questions.length,
                    timeTakenSeconds: durationSeconds
                }).catch(e => console.warn('Faculty email trigger failed:', e));
            }
            ...
            alert('Failed to save submission. Please check your internet connection.');
        }
    };
                ...
                {isScheduledInFuture() ? (
                    ...
                    <div className="space-y-4">
                        <div className="space-y-1">
                            <label className="text-[11px] font-bold text-slate-600 uppercase tracking-wider block px-1">Hall Ticket / Roll Number</label>
                            <input
                        ...
                        <div className="space-y-1">
                            <label className="text-[11px] font-bold text-slate-600 uppercase tracking-wider block px-1">Email Address (for Score & Feedback delivery)</label>
                            <input
                                type="email"
                                className="w-full bg-slate-50 border border-slate-300 rounded-xl px-4 py-3 outline-none text-sm text-slate-800 focus:border-[#b01c1e]"
                        ...
                        <button
                            onClick={handleStart}
                            className="w-full mt-2 bg-[#b01c1e] hover:bg-[#851415] text-white font-bold py-3.5 rounded-xl transition-all shadow-md text-sm cursor-pointer"
                        >
            ...
            <div className="flex-grow w-full bg-white p-4 sm:p-8 rounded-3xl border border-slate-200 shadow-lg overflow-auto relative">
                ...
                <div className="mt-8 flex justify-center">
                    <button
                        ...
                        className="px-8 py-3 bg-[#b01c1e] hover:bg-[#851415] text-white font-bold rounded-xl shadow-md transition-all text-sm cursor-pointer flex items-center gap-2"
                    >
                        Submit Assessment
            ...
            {isBlurred && integrityViolations < 2 && (
                <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4">
                    <div className="max-w-sm w-full bg-white p-6 rounded-3xl border border-red-200 shadow-2xl text-center">
                        <h2 className="text-xl font-black text-red-600 mb-2">Integrity Warning</h2>
                        <p className="text-slate-600 text-xs mb-4">
                            You switched tabs or lost window focus. Your faculty will see tab switch alerts on your submission.
                        </p>
                        <button
